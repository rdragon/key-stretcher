// Modifications with respect to the original file 'ScryptEncoder.cs.original':
// - Added multithreading support
// - Added custom salt support
// - Removed compare methods
// - Removed salt generation
// - Other minor changes
...
namespace Scrypt
{
    ...
    public class ScryptEncoder
    {
        ...
        {
            return EncodeV2(password, salt, _iterationCount, _blockSize, _threadCount);
        }
        ...
        private static string EncodeV2(string password, byte[] saltBytes, int iterationCount, int blockSize, int threadCount)
        {
            ...
            if ((ulong)r * (ulong)p >= 1 << 30 || r > int.MaxValue / 128 / p || r > int.MaxValue / 256 || N > int.MaxValue / 128 / r)
            {
        ...
        public unsafe static byte[] CryptoScrypt(byte[] password, byte[] salt, int N, int r, int p, int keyLength = 32)
        {
            ...
            /* 2: for i = 0 to p - 1 do */
            Parallel.For(0, p, new ParallelOptions() { MaxDegreeOfParallelism = MaxDegreeOfParallelism }, i =>
            {
                var Va = Rent();
            ...
            byte[] Rent()
            {
                if (pool.TryPop(out var array))
                {
                    return array;
                }

                return new byte[128 * r * N + 63];
            }
        }
